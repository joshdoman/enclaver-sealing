...
pub async fn encryption_middleware(
    ...
    let (master_secret_key, _) = app_state
        .master_key_pair
        .get()
        .ok_or_else(|| MiddlewareError::new(
            StatusCode::BAD_REQUEST,
            "Master key pair not initialized"
        ))?;
    ...
    let client_public_key = PublicKey::from_slice(client_public_key_header)
    ...
    let encrypted_body = to_bytes(body, usize::MAX).await
        .map_err(|e| MiddlewareError::new(
            StatusCode::BAD_REQUEST,
            format!("Failed to read request body: {}", e)
        ))?;

    let decrypted_body = decrypt_data(&encrypted_body, &shared_secret)
        .map_err(|e| MiddlewareError::new(
            StatusCode::BAD_REQUEST,
    ...
    *request.body_mut() = Body::from(decrypted_body);
    ...
    let response_body = to_bytes(body, usize::MAX).await
        .map_err(|e| MiddlewareError::new(
            StatusCode::BAD_REQUEST,
            format!("Failed to read response body: {}", e)
        ))?;

    let encrypted_response = encrypt_data(&response_body, &shared_secret)
        .map_err(|e| MiddlewareError::new(
